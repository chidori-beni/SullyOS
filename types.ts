...
export enum AppID {
  ...
  Bookroom = 'bookroom', // 书房 — 在 Reeden 里读，回来报进度；与彼方共用书架，进度进角色记忆
...
export interface LauncherUserWidget {
  ...
  fit?: 'cover' | 'contain';
  /**
   * 是否给图片加一层卡片底框（背景 + 描边 + 投影）。默认不加：
   * 用户传的透明 PNG 就是要和壁纸融在一起，底框和投影会把它框成一张卡。
   * 没有图片的空占位不受此字段影响，永远画框，否则根本看不见。
   */
  frame?: boolean;
  /**
 ...
 * | 档 | 例子 | 角色怎么理解 | 进记忆吗 |
 * |---|---|---|---|
 * | `ambient` 环境 | 「（外面下起了雨）」 | **这是正在发生的事**，⛔ 不知道是你写的 | 进 —— 它是剧情的一部分 |
 * | `directive` 指令 | 「让 A 忍住别提昨天的事」 | 这是**只有 ta 能听见的一句提示** | ⛔ **不进** —— 那是后台调度，不是发生过的事 |
 *
 * ## ⛔ 必须以 system 注入，不占用户发言位
 *
 * 旁白**绝不能存成 `role:'user'`**：那样角色会把它当成「你说的话」，
 * 转头回你一句「外面下雨了吗？我看看」——整个效果就废了。
 * `buildMessageHistory` 遇到这个标记会把 role 改成 `'system'` 并套上旁白框。
 *
 * ## ⛔ 环境档的第一红线：绝不能让角色知道这是你写的
 *
 * 用户要的就是「角色只知道**发生了这件事**，不知道是谁让它发生的」。
 * 所以框定文案里**一个字都不能提到用户/机主/系统**，
 * 只说「这一刻正在发生的事」。（已写成测试。）
 */
export interface NarrationMeta {
    narration: true;
    narrationKind: 'ambient' | 'directive';
    /** `directive` 档：说给谁听（角色名）。空 = 在场所有人 */
    narrationTo?: string;
}
 ...
 */
/**
 * 地点的**地貌**（阶段 6.2）。只影响地图上这个框长什么样，**不进提示词**——
 * 它是给你看的，不是给角色看的（那是 `blurb` 的活儿）。
 *
 * ⛔ 可空，且**绝不自动赋值**：猜错地貌比留空难看得多
 * （「墓园」猜成「草木」会很怪）。编辑器里有「按名字猜」的按钮，但那是你点的。
 */
...
export interface WorldRelationship {
    ...
    value: number;
    /**
     * 关系锁（阶段 2.2）。`true` = 这条**有向边**冻结：演绎的 `relationshipDeltas`
     * 既不改 `value` 也不改 `label`。用户手动编辑不受影响 —— 锁是拦演绎的，不是拦人的。
     *
     * **默认不锁**（`undefined`），且随时可切 —— 用户 B 原话「到时候我自己来开关」，
     * 因为「有时候也会期待混乱的关系」，连自己在意的那对要不要锁都看心情
     * （交接说明 §5.3）。所以这里**绝不能写死默认值**，也不能在建镇时一次性定终身。
     *
     * 为什么锁在有向边上、而不是锁一对人：`relationships` 本来就是有向的
     * （A→B 和 B→A 是两条独立记录，「直观看出不对等」是既有设计）。
     * 锁到边这一级，就能表达「他对她怎么变都行，但她对他别动」这种单向保险。
     *
     * 为什么连 `value` 一起冻：只冻 `label` 的话，好感值会继续往一个方向滑，
     * 滑到下次演绎又想改名时才被锁挡住 —— 结果是数值写着「死对头」、名字还挂着
     * 「挚友」的自相矛盾状态。锁的语义按玩家的直觉来：**这条关系这阵子别动**。
     */
    locked?: boolean;
}
 ...
 * **机主住不住在这个小镇里**（阶段 4.1）。
 *
 * ⚠️ 与 `WorldHomeMode`（存在感档位）**正交**：那个管「ta 对镇民有多重要」，
 * 这个管「ta 的人此刻在不在镇上」。
 *
 * ⛔ **必须守住的边界：每半天一次输入，且是单向的。**
 * - ✅ 长度不限：想写多长写多长
 * - ❌ **不能对角色的反应再作回应** —— 写完就收割这半天的结果
 *
 * 一旦允许来回，小镇就退化成一个很慢的聊天软件，且与「见面」功能完全重复。
 * （早期文档写成「每半天只写一句」是**错的**，用户已指出：限制的**不是长度，是回合**。）
 *
 * 这条边界在实现上是**结构性成立**的：大纲被这一轮消费掉就清空，
 * 界面上根本没有「回复」这个入口。别为了方便再加一个。
 */
export type WorldHostPresence =
    /** 不住在镇上（默认）。行为与阶段 4 之前完全一致 */
    | 'absent'
    /** 在场，但这半天什么都不做。不会替 ta 说话或行动，等你下次操作 */
    | 'silent'
    /** 你写一句「这半天在干嘛」，角色们演的时候能看到你、跟你说话、对你的举动作反应 */
    | 'outline'
    /** AI 替你写这半天的行动和台词 */
    | 'ghostwrite';
...
export interface WorldProfile {
    ...
    places?: WorldPlace[];
    /**
     * 小镇地图一行放几个（阶段 6.2）。缺省 2 —— 手机竖屏下 3 个就开始挤了，
     * 但地点多的世界会更想要 3。⛔ 别写死：这纯粹看地点数量和屏幕。
     *
     * 地图上的**先后顺序就是 `places` 数组的顺序** —— 想重排就在世界设置里上下移，
    ...
    /** 机主住不住在这个镇上（阶段 4.1）。缺省 `absent` = 不住，行为与改造前一致 */
    hostPresence?: WorldHostPresence;
    /**
     * 机主这半天的一次输入（阶段 4.1）。**演完这一轮就清空** ——
     * 这是「一次性输入、不许来回」那条边界的实现方式：
     * 用过即弃，界面上也就没有「回复」可点。⛔ 别把它改成累积的历史。
     */
    hostOutline?: { round: number; text: string; byAi?: boolean };
    /**
...
export interface DateState {
    ...
    /** 最近一次剧情时钟提交来源（如 manual / observation / interlude）。 */
...
/** 存钱罐流水同步到角色聊天时保存的只读卡片快照。 */
export interface BankExpenseCardData {
    version: 1;
    source: 'bank';
...
export interface CharacterProfile {
  ...
  /**
   * 阶段 2.1：**你和 ta 之间**具体是什么关系。两栏**有向、可以完全不对等**。
   *
   * 与 `hostRelation`（陪伴/朋友/不认识我）分工：那个粗粒度的决定「认不认识」，
   * 这两栏是**具体内容**，自由文本——必须能承载说不清的、不正当的关系
   * （用户 B 爱玩「user 是第三者」，见交接说明 §6.4）。
   *
   * ```
   * 你怎么看 ta ：「就是个朋友」          🔓 用户手写，演绎绝不改
   * ta 怎么看你 ：「已经在偷偷喜欢了」    🔒 会随剧情变
   * ```
   */
  hostBond?: {
      /**
       * 机主怎么看 ta。**用户手写，演绎永不改写。**
       *
       * ⛔ **不作为「角色知道的事」注入**（用户 2026-09-11 选定）——
       * 告诉 ta「机主只当你是朋友」会让暗恋 / 单相思当场塌掉，ta 会知道自己没戏。
       * 它注入成**叙述分寸约束**：写到机主时别超出这个程度。
       * 角色自己单方面怎么想不受此限 —— 那正是三角戏的来源。
       */
      fromHost?: string;
      ...
      /**
       * 关系锁·机主侧（阶段 2.2）。`true` = 冻结 `toHost`：小镇演绎和印象重算
       * 都不再改写「ta 怎么看你」，**只有用户自己动手能改**
       * （`applyBondChange` 里 `source === 'manual'` 放行）。
       *
       * 默认不锁，随时可切 —— 同 `WorldRelationship.locked` 的理由。
       *
       * `fromHost`（你怎么看 ta）**不需要锁**：它本来就只有用户能写，演绎永不碰。
       */
   ...
   */
  friendshipSuggestDismissed?: boolean;
  /**
   * 阶段 2.8：**你们是怎么认识的**——角色自己的认知，会注入提示词。
   *
   * 用户 2026-09-10 原话：「我希望这段记忆角色能拥有，比如我们是在彼方上熟悉起来，
   * 然后加了联系方式，变成可以私聊的关系 ⬅️ 在角色的认知中是这样的。」
   *
   * 注意这**不是**可见性控制：用户很清楚自己随时能和任何角色聊天
   * （「毕竟我是最高维的」），要的是**仪式感与来历**，不是把人挡在信息 App 外面。
   */
  acquaintance?: {
      /** 确立为朋友的时刻 */
      at: number;
  ...
  /**
   * 「陪伴，不监督」边界：**默认开**（`undefined` 视为 true）。
   *
   * 一句话：别把角色写成催你写作业的家长 —— 你说「我在洗漱」多半是在分享近况，
   * 不是把待办交给 ta 管理。文案在 `utils/companionshipBoundary.ts`。
   *
   * ⚠️ **一个开关同时管聊天和通话**（用户 2026-09-16 选定）。
   * 分成两个开关的话，会出现「我在聊天里关了，打电话过来还是催我」——
   * 同一个角色两种性格。
   *
   * ⚠️ 这条边界原先**只在通话里、且无条件生效**；改成跟随开关后默认开，
   * 所以**通话行为不变**，变的是聊天那边多了这条。
   *
   * 想要一个会唠叨的角色（妈妈 / 教练 / 担心你身体的恋人）就把它关掉。
   */
  companionshipBoundaryEnabled?: boolean;
...
export interface SocialPost {
    ...
    imagePrompt?: string;
    /** 这张配图是否明确表现角色本人；缺省按普通图片处理，避免误套角色衣橱。 */
    imageIntent?: 'selfie' | 'image';
...
export type MessageType = 'text' | 'image' | 'emoji' | 'voice' | 'collaboration_file' | 'interaction' | 'transfer' | 'system' | 'social_card' | 'chat_forward' | 'xhs_card' | 'score_card' | 'music_card' | 'mcd_card' | 'luckin_card' | 'html_card' | 'news_card' | 'vr_card' | 'trpg_card' | 'novel_card' | 'world_card' | 'sim_card' | 'phone_card' | 'webpage_card' | 'theater_card' | 'room_card' | 'life_card' | 'expense_card' | 'group_topic_card' | 'schedule_invite' | 'schedule_invite_reply';
