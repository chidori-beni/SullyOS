...
日程条与条之间允许有空档（每条都带结束时间，深夜那个洞天天都在）。角色落在空档里时，
`resolveScheduleSlots(...).current` 正确地返回 `null`。这时候写

```ts
// ✗ 错：`??` 分不清「没有快照」和「快照说此刻没有当前时段」
const current = scheduleContext?.current ?? resolveScheduleSlots(schedule, now).current;
```

就会一头栽进兜底分支——而兜底那条通常拿的是**绝对时刻**（`now`），按设备时区读小时，
跨时区角色等于被悄悄换回了用户的钟。空档以外的时间它表现完全正常，所以极难复现。

2026-09-23 的现场：用户在东京、角色 `Asia/Shanghai`，睡眠时段从角色当地 03:38 起。
角色那边 02:38 还在空档里，兜底却按设备的 03:38 命中了睡眠时段，回了一条
「[自动回复]睡了」。回归守卫见 `utils/busyAutoReply.test.ts`「跨时区角色落在日程空档里时」。
...
这里要区分两个时间：`DailySchedule.date` / `planningMeta.targetDate` 表示这张表属于哪一个角色当地日历日，`generatedAt` 只表示它实际创建的时刻，不能单独用后者把跨午夜的预排表判成过期。新生成的预排表会在 `planningMeta` 写入 `planningAhead` 和 `targetDate`；已上传的旧版无标记预排只按严格条件兼容读取，不提前参与旧 key 迁移，也不回写标记。
