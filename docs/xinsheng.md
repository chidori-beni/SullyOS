...
- **排序**：可以按导入顺序正序（先导入 → 后导入）或倒序（后导入 → 先导入）查看，置顶预设始终在最上面；上次选择的排序会自动记住
...
如果后台 / 自然主动生成时出现“全文有字，但卡片样式和内容字段对不上”，打开那条心声后点
「换预设」即可从预设库重新选择显示样式；全文页也有这个入口。它只改变这一条历史的布局和 CSS，
不会重新生成文字、修改角色默认设置或改写全局预设。保存的是当时的渲染快照，所以预设后来被编辑或删除，
这条历史仍能按已保存的快照显示。

「固定为角色当前设置」同样会把操作当下的布局 / CSS 保存到这条历史。这个功能是错配后的恢复手段，
手动覆盖不会抹掉原本的生成快照，随时可以点「恢复生成时样式」撤销。它不代表后台生成端的预设选择
竞态已经被根治；若同类问题持续出现，仍需另行记录生成来源与预设快照排查。
