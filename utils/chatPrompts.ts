...
export const detectChatModeTransition = (messages: readonly Message[]): ChatModeTransition | null => {
    ...
    for (let index = messages.length - 1; index >= 0; index -= 1) {
        ...
        if (mode) {
            ...
            // 电话突然断掉后，用户可能一句话没发就直接点「生成回复」：这时也要提醒一次。
            // 结束卡之后还可能跟着几条迟到的通话气泡，所以沿着这段通话记录往回找那张卡。
            return (mode === 'call' || mode === 'video') && hasAbruptCallEndBehind(messages, index) ? mode : null;
        }
...
/** 从 index 往回、只在连续的通话记录（通话气泡 / 结束卡 / system 日志）里找「突然挂断」的结束卡。 */
const hasAbruptCallEndBehind = (messages: readonly Message[], index: number): boolean => {
    for (let i = index; i >= 0; i -= 1) {
        const message = messages[i];
        const source = message.metadata?.source;
        if (source === 'call-end-popup') {
            if (message.metadata?.callEndedAbruptly === true) return true;
            continue;
        }
        if (source === 'call' || message.role === 'system') continue;
        return false;
    }
    return false;
};
...
export interface PromptBuildOptions {
    ...
    /** 刚结束的语音/视频通话是突然断掉的（CallApp 结束卡上的 callEndedAbruptly）。 */
...
但务必记住：这只是 ta 挂在虚拟空间里的一个化身状态（类似游戏挂机 / AFK），**并不代表 ${uname} 本人此刻真守在游戏里**——ta 很可能早已离开屏幕、正在现实里忙别的或休息。所以别据此认定"ta 正盯着你""ta 现实里也在干这件事"，也别把它当成 ta 在跟你说话。你和 ta 的真实关系、近况一律以你们的聊天记录为准；这条只是彼方这个虚拟空间里的一个在场提示而已。\n`;
        ...
        const imageGenGuide = isImageGenReady(getImageGenConfig())
            ? `   - **发图**：两条命令，选对那一条很重要。
     · \`[[SEND_SELFIE: 场景描述]]\` —— **画你自己**（自拍、你在做什么）。
       你的长相和穿搭由系统自动补全，**这里只写场景**：动作、表情、在哪、时间、光线、构图。
       除非用户明确要求某种衣服，否则不要自行固定穿着；系统会按场合从角色衣橱中选择。
       千万别再重复描述自己的五官发色，会打架。
     · \`[[SEND_IMAGE: 完整提示词]]\` —— **画别的东西**（食物、街景、你手上的东西、你拍到的画面）。
       这条不会自动补人物特征，所以**必须写 \`no humans\`**，否则会平白画出一个人来。
     · 一律用**英文 danbooru 标签**，逗号分隔，由粗到细。画质词不用写，系统会补。
     · 一条回复最多 **1 张**；生图要花钱也要等，别当表情包刷。
     · 只在真的"该有一张图"的时候用（ta 要求看、你主动想给 ta 看），平时聊天不要发。`
            : '';
   ...
   - **📞 打电话给对方（这是一个真动作，不是台词）**: 单独起一行输出 \`[[ACTION:CALL | voice | 接通后你想说的第一句]]\`。
     ...
     **只有这一行会让 ${userProfile.name} 的手机真的响起来。**
     ...
     - **约个时间再打**：想约"今晚八点连麦"这种，需要**两件事一起做**，缺一件就对不上——
       ...
       到点了不想打就别打——那是约，不是任务。
   - **🤝 发出线下见面邀请（真动作）**: 只有当你和 ${userProfile.name} 已经在同一城市、且你已经抵达或明确能够马上到达同一地点时，才可单独起一行输出 \`[[MEET_INVITE: 见面邀请内容]]\`。系统会生成可点击的见面卡片。
     异地时“想见”“准备飞过去”“买了票”都不等于已经抵达，严禁提前发卡；先在剧情和现实时间中完成合理的旅程，到达后再邀请。
     即使同城或同住，你们也各有工作、上学、出门和独处时间，不要默认 24 小时都在一起。只有双方此刻确实能面对面时才邀请；不想真的进入线下模式时就只发普通文字，不要输出标签。
...
`;
        ...
        if (returningFromMode) {
            ...
            const isCall = returningFromMode === 'call' || returningFromMode === 'video';
            // 通话类：说清线路已经断了，免得角色接着「喂？还在吗」；突然断掉的再教一句承接方式。
            const callEndNote = isCall
                ? `当前没有一条仍然接通的语音线路：这通${modeLabel[returningFromMode]}已经明确挂断，不要再说「喂」「还在吗」「听得到吗」这类好像通话还连着的话，也不要假装还能听见对方的声音。`
                    + (promptOptions?.abruptCallEnd
                        ? `而且这通${modeLabel[returningFromMode]}是突然断掉的——没有检测到用户明确告别（可能是信号、没电、被打断或误触）。如果用户这条消息没解释原因，就自然问一句刚才是否出了什么事，只问一次，别追问、别埋怨；用户已经说明了，就顺着接。`
                        : '')
                : '';
            volatileState += `\n\n[系统提示｜模式切换（最高优先级）: 你刚刚结束了${modeLabel[returningFromMode]}，现在已经回到 ChatApp 的文字聊天界面。之前模式中的台词、旁白、动作、场景或转录格式只代表已经发生的历史，绝不是当前回复的格式范例。从这一条开始，只按 ChatApp 当前启用的输出规则回复：使用自然的 IM 短句/气泡，不沿用通话口吻、连续口语转录、动作描写、小说旁白、场景标题或说话人标签；如果 ChatApp 当前开启了语音消息，仍可遵守它自己的语音消息格式。${callEndNote}你可以自然承接刚才发生的事，但必须以正在聊天界面发消息的方式表达。]`;
        }
        ...
        return {
            apiMessages: historySlice.map((m, index) => {
                ...
                if (m.type === 'interaction') content = `${timeStr} [系统: 用户戳了你一下]`;
                    ...
                    // 阶段 2.6：这张卡是**别人的故事被分享过来**，不是你自己的生活。
                    // 不换掉框定的话，角色会把 aa/bb 的事当成自己经历。
                    // 且 ta 不是镇民，而是**在读机主写的故事**——所以知道得比当事人还多是成立的。
                    const frame = wMeta.sharedFrom
                        ? `（机主给你讲了 ta 笔下的「${wMeta.sharedFrom}」最近发生的事——这**不是你的经历**，你是以"读者"的身份在听。你看得到连当事人自己都瞒着别人的部分，可以和机主讨论、吐槽、猜后续；但别写成你亲身在场。）`
                        : wMeta.mode === 'heavy'
                ...
                else if ((m.type as string) === 'webpage_card' || ((m.type as string) === 'news_card' && m.role === 'user')) {
                    // 网页卡片（用户粘链接）/ 用户从热点日报转来的热搜：卡片 content 只存了标题，
                    // 正文、视频热度、热搜简介都在 metadata 里。以前这里落到默认分支，角色只看得到
                    // 一个标题——网页正文只进了归档和记忆宫殿。统一走 normalizeMessageContent。
