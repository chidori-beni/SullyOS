...
describe('collaboration sidecar wiring', () => {
  ...
  it('opens every collaboration file through the forced-share pipeline', () => {
    const windowSource = read('features/collaboration/CollaborationWindow.tsx');
    ...
    // 上游这条还顺带断言 apps/Settings.tsx 也走同一个 shareOrDownloadBlob。
    // 本 fork 的设置页导出是自己一套（直接用 Capacitor Filesystem / Share），
    // 没有改成走 shareExport，所以那半条断言在这里不成立，已移除。
    // 这里保的仍是原来的重点：协同文件一律走强制分享那条路。
  });
