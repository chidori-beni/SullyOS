...
import { getCallSessionWindowForChar } from './callSessionLifecycle';
import {
...
/**
 * 自然主动消息若在电话期间已经由 Worker 生成，可能在挂断后才从 push/outbox 到达。
 * 这类消息的 prompt 快照仍是通话现场，落库后会让角色继续说“我不挂 / 手机开着”。
 * Worker 的 chat_presence 闸门负责主路径，这里只做最后一道收件箱兜底，并且只针对
 * 明确标记为 natural proactive 的后台消息，不吞掉用户主动排的 force 提醒或即时聊天。
 */
export const isStaleNaturalMessageFromCall = (message: Pick<ActiveMsg2InboxMessage, 'charId' | 'source' | 'metadata' | 'sentAt'>): boolean => {
  if (message.source !== 'scheduled' || message.metadata?.amsgNaturalProactive !== true) return false;
  const sentAt = message.sentAt;
  if (typeof sentAt !== 'number' || !Number.isFinite(sentAt) || sentAt <= 0) return false;
  const callWindow = getCallSessionWindowForChar(message.charId);
  if (!callWindow) return false;
  const endAt = callWindow.endedAt ?? Date.now();
  const clockSkew = 2_000;
  return sentAt >= callWindow.startedAt - clockSkew && sentAt <= endAt + clockSkew;
};
...
const flushInboxToChatImpl = async () => {
  ...
  for (const message of pendingMessages) {
    ...
    try {
      ...
      if (isStaleNaturalMessageFromCall(message)) {
        log.warn('通话期间生成的自然主动消息在挂断后迟到，按已过期丢弃', {
          messageId: message.messageId,
          charId: message.charId,
          sentAt: message.sentAt,
          callSessionId: getCallSessionWindowForChar(message.charId)?.sessionId,
        });
        activeMsgTrace('runtime-call-natural-dropped', {
          messageId: message.messageId,
          charId: message.charId,
          sentAt: message.sentAt,
          callSessionId: getCallSessionWindowForChar(message.charId)?.sessionId,
