  ...
  async scheduleCharacterTask(params: {
    ...
    const payload: Record<string, any> = {
      ...
      metadata: {
        ...
        // 自然主动不带 expirePolicy（走自己的判定）；amsgAnchorMs 已随上游
        // 101aed8e「防穿帮闸只管到点那会儿在不在聊天」退休，这里不再写。
  ...
  async syncChatPresence(charId: string, presence: AmsgChatPresence): Promise<void> {
    ...
    const response = await raceWithTimeout(
      client.putClientState([{
        ...
        // 行的版本号走水位（上游 3da0c1e3：只往前走，设备时钟被改过也不会写出未来时间戳），
        // 而不是 presence.activeAt —— worker 读的是 value 里那个 activeAt（45s TTL 判活跃），
        // 版本号只管「这一行能不能盖上去」，两者不是一回事。
        // 这里也不能复用 activeAt：`activeAt: 0` 是下线哨兵，拿它当版本号会让下线那一写
        // 看起来比现有的前台租约更旧，服务端直接跳过，陈旧租约就一直活着。
