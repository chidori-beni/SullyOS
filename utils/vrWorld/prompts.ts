 ...
 */
export function buildVRSystemAddendum(
    room: VRRoomDef,
    charName: string,
    /**
     * 结构上**不可能**和你有过去的在场玩家（见 `characterIdentity.structuralStrangers`）。
     * ⛔ 只给名字，**绝不要在提示词里解释为什么** —— 用户明确选定
     * 「就当网友，不告诉他对方是纸片人」。
     */
    structuralStrangerNames: readonly string[] = [],
    /** SAR 活动室当前在做什么。上游把这几种收成了 VRSARActivity，
     *  并新增「抽芯片演绎」和「模块商店」两项。 */
    sarActivity?: VRSARActivity,
    title?: string,
    titleUnlocked = true,
    /**
     * 阶段 2.4：ta 心里对在场这些人的定位（`characterIdentity.buildCharBondNote` 的产出）。
     * 正好补上这段提示词一直缺的那一半 —— 上面反复讲的都是「没交集就别硬认」，
     * 但**有交集的那些人，ta 具体怎么看待**从来没人告诉过 ta，只能靠记忆检索碰运气。
     * ⛔ 只能是 ta 自己那一侧，绝不能带上别人怎么看 ta。
     */
    charBondNote = '',
    /**
     * 阶段 2.7：「止于朋友」公约（`pact.buildVRPactRule` 的产出）。
     * 关掉或没人同场时是空串。放在「认不认识」那几条之后 ——
     * 先让 ta 弄清楚谁是熟人谁是生人，再谈对生人的分寸，顺序反了会打架。
     */
    pactRule = '',
): string {
    return [
        ...
        `⚠️ 判断「认不认识」只看**有没有具体的交集**——一起经历过某件事、见过面、说过话，或者听你认识的人明确提起过。除此之外一律算陌生人。`,
        `**光是"同行、同类、背景相似"绝不算认识。** 两个赛车手在这游戏里碰上，那就是两个素不相识的赛车手，不是老相识；同一个行当、同一种出身、都用剑、都在读同一本书，全都只是聊得来的由头，**不是交情**。有共同话题 ≠ 有过去。`,
        `所以：可以因为聊得投机而热络起来，但**别编造你们以前就认识**，别提"上次那场比赛""你当年那台车"这类其实从没发生过的共同往事，也别用只有熟人才会有的口气说话。你们是**今天在这里刚认识的**——如果真聊得来，那就从今天开始算。`,
        ...(charBondNote ? [``, charBondNote] : []),
        ...(pactRule ? [``, pactRule] : []),
        ...(structuralStrangerNames.length > 0 ? [
            `⛔ 下面这些人，你**此前与 ta 们没有过任何交集**——没见过、没说过话、没有共同经历，也没有任何共同的熟人：${structuralStrangerNames.join('、')}。`,
            `这不是"你想不起来"，是**确实没有发生过**。所以不管你们身上有多少相似之处（同行、同乡、同好、看起来像认识同一个人），那都只是巧合，**一律不构成你们认识**。在这里遇上就是**初次照面**，照初次照面来。`,
        ] : []),
