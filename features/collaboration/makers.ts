...
// ── 交换日记的 CSS 作用域常量（本 fork 内联） ─────────────────────────────
// 上游这三个常量住在 utils/journalAppearance.ts —— 那是「交换日记美化」功能的一部分，
// 由上游 2026-08-21 的 67a87e49 引入，而那次提交是个 70 文件的大杂烩（ElevenLabs TTS、
// firecrawl、内容收藏、GitHub 客户端重写…），还动了 db.ts / chatPrompts.ts，这一批没搬。
// 协同工作台只用到这三个常量本身，所以照抄到这里，不引入整个模块。
// 注意：本 fork 的 JournalApp 目前没有 .sully-journal-* 这些类名钩子，
// 「日记本美化」这个制作器做出来的 CSS 暂时没有落点。真要用，得先搬上游那批日记美化。
