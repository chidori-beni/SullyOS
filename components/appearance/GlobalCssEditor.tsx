...
export interface GlobalCssPatch {
  css: string;
  presets: CssPreset[];
  presetId?: string;
}
...
type Props = {
  ...
  onPatch: (patch: GlobalCssPatch) => void;
...
const GlobalCssEditor: React.FC<Props> = ({ value, presets, activePresetId, onPatch, onNotify }) => {
  ...
  const handleSave = () => {
    const name = window.prompt('给这套弹窗样式起个名字：', `弹窗样式 ${presets.length + 1}`);
    if (name === null) return;
  ...
  const handleDelete = (preset: CssPreset) => {
    if (!window.confirm(`删除「${preset.name}」这套弹窗样式？`)) return;
    onPatch({
      css: value,
  ...
  const handleCopyPrompt = async () => {
    if (await copyText(GLOBAL_CSS_AI_PROMPT)) {
      setCopied(true);
    ...
    <div className="space-y-3">
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {BUILTIN_GLOBAL_CSS_PRESETS.map(preset => (
            <button
              key={preset.name}
      ...
      <div>
        <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2">
          <span className="text-[11px] font-bold text-slate-500">
            CSS 代码 <span className="font-normal text-slate-400">· 整机生效</span>
          </span>
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50/70">
        ...
        >
          <span className="text-[11px] font-bold text-slate-500">
            有哪些类名 <span className="font-normal text-slate-400">· {UI_HOOK_CATALOG.length} 个</span>
          </span>
          <span className="text-[10px] text-slate-400">{catalogOpen ? '收起' : '展开'}</span>
        </button>
        {catalogOpen && (
          <ul className="space-y-0.5 px-3 pb-3">
            {UI_HOOK_CATALOG.map(entry => (
