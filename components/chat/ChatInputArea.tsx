...
interface ChatInputAreaProps {
    ...
    /** 私聊内置加号菜单的顺序；群聊的 actionsContent 不走这份排序。 */
    builtInActionOrder?: readonly ChatActionId[];
    ...
    /** 心声（per-character）。开着时按钮上打个点，跟「思考已开」一个观感。 */
...
const ChatInputArea: React.FC<ChatInputAreaProps> = ({
    ...
    actionsContent,
    builtInActionOrder = DEFAULT_CHAT_ACTION_ORDER,
    ...
    // 发送按钮样式沿用上游三档写法（圆形 / 胶囊 / 极简），这样社区美化对它的选择器和预期一致。
                                    ...
                                    {visibleEmojis.map((e) => {
                                        ...
                                        >
                                            <div className="aspect-square w-full">
                                                ...
                                                <img key={e.url} src={e.url} loading="lazy" decoding="async" className="sully-emoji-thumb w-full h-full object-contain pointer-events-none" />
                                            </div>
                    ...
                    {showPanel === 'actions' && !actionsContent && (
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            /* 「心声」按钮：打开自定义面板（含总开关）。卡片本身从消息头像点进去。 */
                            xinsheng: (
                            <button
                              onClick={() => onPanelAction('xinsheng-settings')}
                              className={`flex flex-col items-center gap-2 active:scale-95 transition-transform ${acnh ? 'text-[#725d42]' : isDiscordStyle ? 'text-slate-200' : 'text-slate-600'}`}
                            >
                              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm border relative ${
                                  xinshengEnabled
                                    ? (isDiscordStyle ? 'bg-rose-500/20 text-rose-300 border-rose-400/40' : 'bg-rose-100 text-rose-500 border-rose-200')
                                    : (isDiscordStyle ? 'bg-slate-800 text-rose-300 border-rose-400/20' : 'bg-rose-50 text-rose-400 border-rose-100')
                              }`}>
                                  <Heartbeat className="w-6 h-6" weight="bold" />
                                  {xinshengEnabled && <span className={`absolute -top-1 -right-1 w-3 h-3 rounded-full border-2 ${isDiscordStyle ? 'bg-rose-400 border-slate-900' : 'bg-rose-500 border-white'}`} />}
                              </div>
                            ...
                            voice: onOpenVoiceInput && !showVoiceButton ? (
                            <button
                            ...
                            >
                              <span className={`w-14 h-14 rounded-2xl grid place-items-center shadow-sm border ${acnh ? 'bg-white/70 border-[#e6dab4] text-[#c17b42]' : isDiscordStyle ? 'bg-slate-800 text-amber-300 border-amber-400/20' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>
                                <Star className="w-6 h-6" weight="bold" />
                              </span>
                              <span className="text-xs font-bold">收藏</span>
                            </button>
                            ),
                            };
                            const items = normalizeChatActionOrder(builtInActionOrder)
                                .map(id => builtInActions[id]
                                    ? <React.Fragment key={id}>{builtInActions[id]}</React.Fragment>
                                    : null)
