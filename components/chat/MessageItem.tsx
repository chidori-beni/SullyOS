...
import { readImageGenMeta, retryImageGeneration, type AppearanceSelectionApi } from '../../utils/novelaiImage';
...
const MessageItem = React.memo(({
    ...
    const renderAvatar = (
        ...
        return (
            <div
                className={`${visible ? 'sully-chat-avatar-wrap ' : ''}relative ${avatarSizeClass} z-0 ${options?.className || ''}${avatarClickable ? ' cursor-pointer active:scale-95 transition-transform' : ''}`}
    ...
    if (isSystem) {
        // 只给用户看的提示（UiNoticeMeta）：留在聊天窗口里，但**不进角色上下文、不进记忆**
        // （排除点在 ChatPrompts.buildMessageHistory 与 isMessageSemanticallyRelevant）。
        // 用户要的就是这个形态 —— 不弹窗（会错过），也不让角色在台词里演。
        if ((m.metadata as any)?.uiNotice) {
            return (
                <div className={`${cardShellClass} w-full ${selectionMode ? 'pl-8' : ''} animate-fade-in`} {...cardHook}>
                    <div className="w-full px-6 my-2.5" {...interactionProps}>
                        <div className="mx-auto flex max-w-sm items-start gap-1.5 rounded-xl bg-slate-500/[0.07] px-3 py-2">
                            <span className="text-[11px] leading-[1.45] text-slate-400 shrink-0">·</span>
                            <span className="text-[11px] leading-[1.45] text-slate-400">
                                {typeof m.content === 'string' ? m.content : ''}
                                <span className="block text-[9.5px] text-slate-300 mt-0.5">只有你看得到，ta 不知道有这条</span>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                {m.content ? (
                    ...
                    <div className="px-4 py-3 rounded-2xl bg-slate-100 text-slate-500 text-xs min-w-[140px] max-w-[220px] space-y-1.5">
                        ...
                        <button
                            type="button"
                            onClick={() => {
                                void retryImageGeneration(m.id, imageGenMeta.prompt, m.charId, {
                                    imageIntent: imageGenMeta.imageIntent,
                                    selfieScene: imageGenMeta.selfieScene,
    ...
    const stripJunk = (s: string) => stripFaceToFacePhoneSourceTags(stripMessageReactionTags(stripFishCuesForDisplay(stripLeakedSourceTags(s
        ...
        .replace(/\[\[(?:ACTION|RECALL|SEARCH|DIARY|READ_DIARY|FS_DIARY|FS_READ_DIARY|SEND_EMOJI|SEND_IMAGE|SEND_SELFIE|DIARY_START|DIARY_END|FS_DIARY_START|FS_DIARY_END)[:\s][\s\S]*?\]\]/g, '')
