  ...
  return <Modal isOpen={isOpen} title="自然主动" onClose={onClose} footer={<>
    ...
    <div className="space-y-5">
      ...
      {enabled && <>
        ...
        <div className="rounded-2xl bg-slate-50 p-3 border border-slate-100"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="text-xs font-bold text-slate-600">{profile ? `联络画像：${profile.archetype}` : '首次开启会阅读角色人设'}</div><p className="text-[11px] text-slate-400 leading-relaxed mt-1">{profile?.summary ?? '只读取一次并保存画像，不会每次检查都烧一遍模型。'}</p></div>{profile && <button disabled={busy} onClick={() => void save(true)} className="shrink-0 text-[11px] font-bold text-violet-500 px-2 py-1.5 bg-white rounded-lg border border-violet-100">重新理解</button>}</div></div>
