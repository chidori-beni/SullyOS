...
const Messaging: React.FC = () => {
    ...
    const callMomentWriter = async (selected: CharacterProfile[], count: number): Promise<any[]> => {
        ...
        const prompt = `你正在为一个拟真的朋友圈生成角色动态。只能使用下面角色，不得代替用户发言。\n\n${roster}\n\n请生成 ${count} 条动态，返回严格 JSON 数组。每项必须包含 charId、text、imagePrompt、imageIntent、location。text 是角色自愿公开发布的自然朋友圈文案；imagePrompt 是与正文一致、适合生图 API 的英文画面提示词，如果这条动态不适合配图则填空字符串；imageIntent 只能是 "selfie" 或 "image"：只有角色本人出镜、自拍或被拍时填 "selfie"，食物、景物、物品、陌生人或不出镜的内容填 "image"；location 可为空。不要 Markdown，不要解释。`;
        const response = await fetch(`${apiConfig.baseUrl.replace(/\/+$/, '')}/chat/completions`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiConfig.apiKey}` },
            body: JSON.stringify({
        ...
        try {
            ...
            for (let index = 0; index < raw.length && nextPosts.length < requestedCount; index += 1) {
                ...
                if (imagePrompt && canDraw) {
                    ...
                    try {
                        const resolvedPrompt = imageIntent === 'selfie'
                            ? await buildSelfiePromptForGeneration(char.id, imagePrompt, imageConfig, apiConfig, {
    ...
    const rerollMomentImage = async () => {
        ...
        try {
            const promptCharacter = post.authorType === 'character' && post.authorCharId
                ...
                : undefined;
            const resolvedPrompt = post.imageIntent === 'selfie' && post.authorType === 'character' && post.authorCharId
    ...
    return (
        <div ref={appRef} className="sully-messaging-app" onClick={() => { if (contextMenu) setContextMenu(null); if (momentActionsPostId) setMomentActionsPostId(null); }}>
            ...
            {profileEditOpen && <div className="sully-msg-modal-layer sully-msg-profile-editor" onClick={() => setProfileEditOpen(false)}><div className="sully-msg-modal-card sully-msg-modal-card-tall" onClick={event => event.stopPropagation()}>
                ...
                <div className="sully-msg-profile-images"><button onClick={() => profileAvatarInputRef.current?.click()}><img className="sully-msg-profile-avatar-preview" src={profileDraft.avatar || userProfile.avatar} alt="" /><span>更换头像</span></button><button onClick={() => profileCoverInputRef.current?.click()}>{profileDraft.cover ? <img src={profileDraft.cover} alt="" /> : <span className="sully-msg-cover-placeholder">添加封面</span>}<span>更换封面</span></button></div>
