...
import { synthesizeSpeechDetailed, canSynthesizeSpeech, characterHasVoice, cleanTextForTtsProvider, stripTtsMarkupForDisplay, providerUsesRawVoiceMarkup } from '../utils/ttsRouter';
...
/**
 * 「能在聊天界面翻到的消息」—— 跳转定位算下标和渲染切片必须共用这一份。
 * 两边各写一份过滤条件的话，稍有出入就会下标错位、跳过去落在别的消息上。
 */
const browseableChatMessages = (messages: Message[], hideSystemLogs?: boolean): Message[] =>
    filterChatMessages(messages)
        .filter(m => !m.metadata?.proactiveHint)
        .filter(m => !isHiddenSystemLog(m, hideSystemLogs));
...
const Chat: React.FC<ChatProps> = ({ onBack }) => {
    ...
    /** 旧消息浏览模式下当前渲染哪一段（相对可见消息列表的下标区间）。null = 不在该模式。 */
    ...
    const handleOpenCallRecord = useCallback((charId: string, sessionId: string) => {
        ...
        // CallApp is mounted as a separate PhoneShell app. Preserve this
        // origin so its record-detail back button can return straight to the
        // same Chat screen instead of making the user traverse CallApp first.
        callLaunch.request({ charId, sessionId, returnTo: 'chat' });
        openApp(AppID.Call);
    ...
    /** 往上续载前记下滚动高度，DOM 提交后补偿差值，避免内容被顶得跳走。 */
    ...
    /** 定位动画自己也会触发 onScroll；动画没停之前不许续载。 */
    ...
    // 「装扮」总抽屉：微调 / 背景 / 气泡 / 白框 / 提示音 五件事的统一入口（null = 收起）。
    // 整合前这五项散在加号面板两页 + 设置弹窗里，现在只留「＋ → 装扮」一个入口。
    const [decorTab, setDecorTab] = useState<ChatDecorTab | null>(null);
    ...
    const handleRerollVoice = async (msgId: number) => {
        ...
        if (!isTtsReady()) {
            ...
            addToast(provider === 'fishaudio'
                ...
                : provider === 'elevenlabs'
                    ? '该角色未配置 ElevenLabs Voice ID 或缺少 ElevenLabs Key'
    ...
    const extendHistoryWindow = useCallback((direction: 'older' | 'newer') => {
        ...
        if (direction === 'older') {
            // 前插消息会把当前内容整体往下顶；记下原高度，DOM 提交后补偿差值，
            // 用户眼里的位置就不会突然跳走。
    ...
    const handlePanelAction = (type: string, payload?: any) => {
        ...
        if ([
            ...
            'html-mode-toggle', 'html-mode-settings', 'thinking-settings', 'xinsheng-settings', 'favorites', 'collaboration',
        ...
        switch (type) {
            ...
            // （设置弹窗跳转、历史入口）不用跟着改。
            case 'decor': setShowPanel('none'); setModalType('none'); setDecorTab((payload as ChatDecorTab) || 'fine-tune'); break;
            case 'chrome-css': setShowPanel('none'); setModalType('none'); setDecorTab('chrome'); break;
            case 'chrome-sound': setShowPanel('none'); setModalType('none'); setDecorTab('sound'); break;
            case 'fine-tune': setShowPanel('none'); setModalType('none'); setDecorTab('fine-tune'); break;
    ...
    const handleHistoryCleanupDone = async (plan: ChatCleanupPlan) => {
        ...
        // 上游这里还会重置「历史窗口」，那是另一批的功能（utils/chatHistoryWindow.ts），
        // 本 fork 没有；下面的 setVisibleCount + reloadMessages 已经把视图归位了。
        setVisibleCount(LOAD_BATCH_SIZE);
        visibleCountRef.current = LOAD_BATCH_SIZE;
    ...
    const handleJumpToMessageInChat = async (messageId: number) => {
        ...
        const browseableMessages = browseableChatMessages(allMsgs, char?.hideSystemLogs);
        ...
        // 等窗口节点挂上 DOM 再定位；定位动画结束后才开放边缘续载，
        // 免得平滑滚动自己触发 onScroll、提前把窗口撑开。
    ...
    // 真正想从聊天记录里抹掉，应该走"删除"。
    // 旧消息浏览模式仍然只挂一个有限窗口，但窗口会随上下滚动持续扩展。
    const chatDisplayMessages = useMemo(
        () => browseableChatMessages(messages, char?.hideSystemLogs),
    ...
    // 注意：这两个 useMemo 必须留在下面 `if (!char) return` 之前——提前返回会跳过它们，
    // hook 数量在两次渲染间对不上，React 会抛 "Rendered fewer hooks than expected" 直接崩。
    // 合上游新增：自动回复（你不说话时按设置自己接话）。
    const autoReply = useChatAutoReply({
        ...
        blocked: isInputFocused || !!input.trim() || showPanel !== 'none' || modalType !== 'none'
            || selectionMode || isSummarizing || collaborationOpen
    ...
    return (
        ...
        >
            ...
            <div ref={scrollRef} onScroll={handleChatScroll} className="sully-chat-scroll flex-1 overflow-y-auto overflow-x-hidden pt-6 pb-6 no-scrollbar" style={{ backgroundImage: activeTheme.type === 'custom' && activeTheme.user.backgroundImage ? 'none' : undefined }}>
                ...
                {collapsedCount > 0 && windowedFocusMsgId === null && (
                    <div className="flex justify-center mb-6">
                        ...
                        }} className="sully-chat-history-more px-4 py-2 bg-white/50 backdrop-blur-sm rounded-full text-xs text-slate-500 shadow-sm border border-white hover:bg-white transition-colors">加载历史消息 ({collapsedCount})</button>
                    </div>
                )}
                {windowedFocusMsgId !== null && (
                ...
                {renderedMessages.map((m, i) => {
                    ...
                    return (
                        ...
                        <MessageItem
                            ...
                            imageSelectionTimeZone={char.customTimezoneEnabled ? char.customTimezone : undefined}
            ...
            {/* 角色专属「聊天装扮」悬浮气泡 + 小面板 —— 从「装扮」抽屉的「微调」页签切过来（也可从加号面板长驱直入）。
            ...
            {/* 「装扮」总抽屉 —— 从加号面板唯一的「装扮」入口进入（旧的「聊天装扮 / 白框 /
                提示音」三个格子和「设置」弹窗里的聊天背景，全部并到这里的五个页签）。
                只占下半屏、不铺遮罩色块：上方真聊天保持可见，白框和微调靠它当实时预览。 */}
            {char && decorTab && (() => {
                const override = char.chatFineTune;
                const customized = override?.enabled === true;
                // 控件展示合并后的生效值：未覆盖的字段显示全局当前值，改哪个才覆盖哪个
                const effective = mergeChatFineTune(osTheme, override);
                // 提示音：默认独立存于 char.chatSound；打开「绑定到白框」则改存进
                // char.chromeCustomCss 的 @sully-sound 指令、随白框分享一起走。
                ...
                return (
                    <ChatDecorSheet
                        charName={char.name}
                        tab={decorTab}
                        onChangeTab={setDecorTab}
                        onClose={() => setDecorTab(null)}

                        fineTuneValue={effective}
                        fineTuneCustomized={customized}
                        onToggleFineTuneCustomized={(next) => updateCharacter(char.id, { chatFineTune: { ...override, enabled: next } } as any)}
            ...
            {char && (
                <React.Suspense fallback={collaborationOpen ? <div className="absolute inset-0 z-[120] grid place-items-center bg-slate-50 text-xs text-slate-400">正在打开协同工作…</div> : null}>
                    <CollaborationWindow
                        ...
                        user={userProfile}
                        theme={activeTheme}
                        backgroundUrl={char.chatBackground}
                        chatApi={apiConfig}
                        apiPresets={apiPresets}
