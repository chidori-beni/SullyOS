...
const isFreeNote = (p:BoardPost) => 'kind' in p && p.kind==='favor' && p.offer===0;
const postKind = (p:BoardPost) => 'price' in p ? '转让' : p.kind==='item' ? '求物' : p.kind==='favor' ? (isFreeNote(p)?'便笺':'求回应') : '收心意';
