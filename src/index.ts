...
export const amsgHooks = {
  async onBeforeFire(ctx: FireCtx) {
    ...
    if (!instant && taskMeta.amsgNaturalProactive === true) {
      ...
      // 自然主动也必须有一道**到点硬闸**。正常情况下 decideNaturalProactive 已经
      // 会因为未回复次数而返回 shouldSend=false，但这里再按热络档位拦一次，防止
      // 旧画像 / 旧 bundle / 并发重跑时把「连续未回复」误当成可以继续发。下一颗
      // 检查已经在上面排好，所以用户回复后计数清零，系统会自然恢复；不会永久停摆。
